    ...
    if FORCE_SUB:
        try:
          ...
          if str(user.status) == "ChatMemberStatus.BANNED":
              await message.reply_text("You are Banned. Contact -- 𝐓𝐞𝐚𝐦 𝐂𝐢𝐧𝐝𝐞𝐫𝐞𝐥𝐥𝐚")
              return 1
        except UserNotParticipant:
            link = await app.export_chat_invite_link(FORCE_SUB)
            caption = f"Join our channel to use the bot"
            await message.reply_photo(photo="https://graph.org/file/3f0529e0a232d7a076a60-998260a13c34d2b7ea.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{link}")]]))
            return 1
...
help_pages = [
    ...
    (
        ...
        "> Check premium plans\n\n"
        "13. **/speedtest**\n"
        "> Test the server speed (not available)\n\n"
        "14. **/terms**\n"
        ...
        "**__Powered by 𝐓𝐞𝐚𝐦 𝐂𝐢𝐧𝐝𝐞𝐫𝐞𝐥𝐥𝐚__**"
    )
]
...
async def terms(client, message):
    ...
    buttons = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("💬 Contact Now", url="https://t.me/SmartBoy_ApnaMS")],
        ]
    )
